...
double arr_lat_rad=LEDA_LAT*(M_PI/180.0),arr_lon_rad=LEDA_LON*(M_PI/180.0),height=LEDA_HGT;
        ...
        for(chan=0; chan < uvdata->n_freq; chan++) {
            ...
            switch(do_flag) {
                ...
                case 1:
                    ...
                    break;
                case 2: 
                    // LEDA 40kHz channels. do not use the n_rej edge channels to compare to neighbours
                    ...
                    break;
                case 3:
                    // LEDA 10kHz channels. do not use the n_rej edge channels to compare to neighbours
                    n_rej = 20;
                    cpc=128;
                    if (chan%cpc < n_rej || chan%cpc >= cpc-n_rej) lower = upper = chan;
                    if (chan%cpc > n_rej && lower%cpc < n_rej) lower += n_rej-lower%cpc;
...
void printusage(const char *progname) {
  ...
  fprintf(stderr,"-A lon,lat \tSpecify East Lon and Lat of array center (degrees). Comma separated, no spaces. Default: LEDA\n");
  ...
  fprintf(stderr,"\t\t2:\tspecial treatment for LEDA edge channels, 40kHz case\n");
  fprintf(stderr,"\t\t3:\tspecial treatment for LEDA edge channels, 10kHz case\n");
